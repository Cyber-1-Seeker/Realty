...
async def check_user_permissions(telegram_id):
    print(f"Проверка прав для telegram_id: {telegram_id}")
    try:
        url = f"{API_URL}/api/accounts/check-permissions/?telegram_id={telegram_id}"
        print(f"URL для проверки прав: {url}")
        async with httpx.AsyncClient() as client:
            res = await client.get(url, headers=API_HEADERS)
            print(f"Статус ответа проверки прав: {res.status_code}")
            print(f"Содержимое ответа проверки прав: {res.text[:200]}...")
            ...
            print(f"Результат проверки прав: {has_permission}")
...
async def list_applications(message: Message, page: int = 1):
    ...
    print(f"Команда /заявки от пользователя с telegram_id: {telegram_id}")
    print(f"Имя пользователя: {message.from_user.first_name}")
    print(f"Username: {message.from_user.username}")
    
    # Проверяем права пользователя
    if not await check_user_permissions(telegram_id):
        print(f"Пользователь {telegram_id} не имеет прав для просмотра заявок")
        return await message.answer("❌ У вас недостаточно прав для просмотра заявок")

    print(f"Пользователь {telegram_id} имеет права, получаем заявки...")
...
async def get_users_to_notify(api_url, api_token):
    ...
    try:
        url = f"{api_url}/api/accounts/role-users/"
        headers = {"Authorization": f"Token {api_token}"}

        async with httpx.AsyncClient() as client:
            response = await client.get(url, headers=headers)
            print(f"Статус ответа: {response.status_code}")
            print(f"Содержимое ответа: {response.text[:500]}...")  # больше символов

            if response.status_code == 200:
                users = response.json()
                print(f"Тип users: {type(users)}")
                print(f"Количество пользователей: {len(users) if isinstance(users, list) else 'не список'}")
                if isinstance(users, list) and users:
                    print("Пример пользователя:", users[0])
                users_with_telegram = [user for user in users if user.get('telegram_id')]
                print(f"Пользователей с telegram_id: {len(users_with_telegram)}")
                if users_with_telegram:
                    print("Пример с telegram_id:", users_with_telegram[0])
